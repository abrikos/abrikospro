...
v-card(v-if="!loggedUser.strategy" width="600" )
    v-card-text
        q-input(v-model="password" :label="$t('New password')" type="password")
        q-input(v-model="password2" :label="$t('Confirm password')" type="password" :error-messages="(password!==password2) ? $t('Password and confirmation must be the same'): null")
    v-card-actions(v-if="canSubmit()")
        q-btn(@click="changePassword") {{$t('Save')}}
